...
const UbahModal = ({ id, ujianId, isAcakSoal, modalRef, getData }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        await axios({
            ...
            if (res.data?.id) {
                modalRef.hide()
                getData()
                Swal.fire('Berhasil', 'Soal berhasil diubah!', 'success')
                setNomorSoalError('')
                setPenilaianError('')
                setPertanyaanError('')
                setJenisError('')
    ...
    useEffect(() => {
        const getCurrentData = async () => {
            await axios({
                method: 'GET',
                url: `${process.env.NEXT_PUBLIC_RESTFUL_API != undefined ? process.env.NEXT_PUBLIC_RESTFUL_API : ''}/soal/soal/${id ? id : ''}`,
            }).then((res) => {
                if (res.data?.id) {
                    setNomorSoal(res.data.nomorSoal)
    ...
    return (
        <ModalBody targetModal={'ubahSoal'} size='lg' title={<h6 className='mb-0'><FontAwesomeIcon icon={faPenAlt} /> Ubah</h6>} footer={<button type='button' onClick={handleSubmit} ref={submitRef} className='btn btn-primary float-end'><FontAwesomeIcon icon={faSave} fixedWidth /> Simpan</button>}>
