...
const Ubah = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        await axios({
            method: 'PUT',
            url: `${process.env.NEXT_PUBLIC_RESTFUL_API != undefined ? process.env.NEXT_PUBLIC_RESTFUL_API : ''}/income/${id ? id : ''}`,
            data: {
                eventId,
                jumlah,
                tanggal
            ...
            if (res.data?.id) {
                Swal.fire('Berhasil', 'Income berhasil diubah!', 'success').then((res) => {
    ...
    return (
        <>
            <Head>
                <title>Ubah Income</title>
            </Head>
            <Template header='Ubah Income' breadCrumb={[{ label: 'Dashboard', link: '/dashboard' }, { label: 'Income', link: '/dashboard/income' }, { label: 'Ubah Income', link: `/dashboard/income/ubah/${id}` }]} breadCrumbRightContent={
